...
def eval(model, data, loss_func, scaler, half=False):
    model.eval()
    loss_list = []
    label_list = []
    pred_list = []
...
'''
def train(model, mparam: Model_param, half=False, slim=False, slim_l1_s = 0.0001):
    ...
    for e in range(mparam.epoch):
        ...
        for data in train_loader:
            ...
            if half:
                with autocast():
                    pred = model(img)
                    # print(f'{pred.__name__}, {type(pred)}, {type(label)}')
                    loss = mparam.loss_func(pred, label)
                mparam.optimizer.zero_grad()
                scaler.scale(loss).backward()
                scaler.step(mparam.optimizer)
            ...
            if count % mparam.eval_steps == 0:
                ev_loss, ev_acc = eval(model, test_loader, mparam.loss_func, scaler, half)
                if ev_acc > max_acc:
                    max_acc = ev_acc
...
import vgg
model = vgg.vgg11(pretrained=False, num_classes=10)
